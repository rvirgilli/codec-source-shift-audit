...
the WIFS 2026 submission's statistical claims from released per-utterance scores.
The reviewer snapshot is tagged `wifs2026-reviewer-artifact`. It does **not** contain
...
benchmark protocol. The ASVspoof 5 and MLAAD-en analyses are also custom diagnostic
re-slices and are reported only as scope checks.

## Submission claim map

| Submission result | Authoritative script | Machine-readable output |
|---|---|---|
...
| ASVspoof 5 and MLAAD scope checks | `src/external_crossed_effects.py` | `outputs/external_crossed_effects_200k.json` |
...
authoritative for the submission claims. In particular, use the tie-correct
Mann--Whitney AUROC in the scripts listed above rather than the legacy ROC-trapezoid
helper in `src/audit_lib.py`. `src/reproduce.py` is retained only for provenance of
superseded drafts.
...
python src/rank_crossed_audit.py --reps 200000 --batch 2000 --output outputs/rank_crossed_audit_200k.json
...
scikit-learn is used only by the optional tie-validation script. The scripts require
Python 3.11 or newer. The crossed analyses were validated with Python 3.13.2 and the
NumPy version pinned in `requirements.txt`.
...
```text
ASVspoof 5 / frozen WavLM: global max-T p=0.217; none familywise positive
...
is not used as confirmatory support in the submission.
...
selected training IDs are absent. This boundary is stated explicitly in the paper and
is part of the released artifact's scope.
...
from ASVspoof 5, MLAAD, and LibriSpeech. Code is MIT licensed. Author-generated
scores and analysis outputs are released under CC BY 4.0; raw audio is not
redistributed here. See `DATA_LICENSE.md` for the path-level licensing boundary.
