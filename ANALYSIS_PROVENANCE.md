...
| Commit | Time | Event | Status for submission |
|---|---|---|---|
...
| `9c708c4` | 2026-07-02 03:37 | ASVspoof 5 second-instrument sweep and outcome | Run predates any public rule file; retrospective |
...
- the crossed reanalysis of the ASVspoof 5 and MLAAD external families.
...
public rule commit that predates its run. The submission treats the external MLAAD
family as a scope check, not as independent confirmation: its crossed global test
is unresolved (`p=0.520`) and no fold survives familywise correction.

## Superseded claims

The old `PREREGISTRATION.md` called the targeted TORTOISE and ASVspoof 5 analyses
pre-registered and reported older seed-focused intervals. The repository history does
not substantiate those timing claims, and the submission does not use those intervals.
The authoritative submission values are in:
